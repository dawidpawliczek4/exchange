...
| D15 | Frontend | Vue 3 + TypeScript + Vite, Pinia for state, Tailwind, TradingView lightweight-charts (Apache 2.0, attribution), native WebSocket. Stream: snapshot + deltas with sequence numbers (Binance pattern), server-side coalescing to 10–30 Hz | Standard, proven stack; 35–60 h. Originally planned as React + Zustand; switched to Vue + Pinia at implementation time, same architecture |
