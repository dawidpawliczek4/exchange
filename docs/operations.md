...
cd frontend && pnpm dev             # trading UI on :5173 with HMR (or ./gradlew :frontend:pnpmDev)
```
...
:frontend:pnpmDev` starts the same server through Gradle's downloaded Node, handy without a
local pnpm) — `./gradlew :frontend:build` exists for CI and images and produces a static
