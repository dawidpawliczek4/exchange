...
### Ledger / deposit — in-memory path works, WAL record missing

ADR-0001 puts balances in the single-writer hot path; the full design (decisions L1–L8, invariants, implementation order) is `docs/ledger-design.md`. The in-memory flow works: `DepositCommand` → writer loop → `Ledger.deposit` (credits cash or rejects) → `DepositAccepted`/`DepositRejected` → `KafkaAccountFeedSink` → `account.events`, covered by `LedgerTest` and a `MatchingRunnerIntegrationTest` case. **The gap is durability**: `WalCodec.encodeDeposit` still returns an empty array with no decode tag — the empty payload is appended with a valid CRC, so a journal containing a deposit **breaks recovery** (decode underflows). Finishing the WAL kind-2 record is the immediate next step; until then no gateway endpoint produces deposits, so production can't hit it. Treat changes here as "finish the feature per ADR-0001", not a drive-by fix.
